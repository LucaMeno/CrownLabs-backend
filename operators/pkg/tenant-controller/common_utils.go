...
func randomDuration(minTime, maxTime time.Duration) time.Duration {
	if minTime >= maxTime {
		return minTime
	}

	return (minTime + time.Duration(rand.Float64()*float64(maxTime-minTime))).Truncate(time.Millisecond) //nolint:gosec // don't need crypto/rand
}
